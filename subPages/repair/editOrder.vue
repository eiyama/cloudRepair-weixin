<template>
  <view style="margin-bottom: 100rpx;">
	  <u-navbar :background="{backgroundColor: '#315ade'}" :is-back="false" title-color="#fff" title="编辑工单">
    ...
    <view class="cenmen">
      ...
      <view class="order_message">
		...
		<u-field :required="true" label="工单来源" label-width="150" :disabled="true" right-icon="arrow-right"
		  v-model="registration.sourceTypeName"  @click="openchoose_type" placeholder="请选择工单来源" >
		</u-field>	
		...
		<u-field :required="true" label="留修类型" label-width="150" :disabled="true" right-icon="arrow-right"
		  v-model="registration.pushRepairTypeName"  @click="opchoose_Ordertype" placeholder="请选择留修类型" >
		</u-field>	
		...
		<view class="" style="margin: 0 10px;">
			<u-form-item label-width="150" label="车辆现状">
				<u-radio-group :shape="square" v-model="current"  @change="radioGroupChange" >
					<u-radio v-for="(item, index) in canMoveCarList"   :key="index" :name="item.value"   >
						{{ item.name }}
					</u-radio>			
				</u-radio-group>
			</u-form-item>
...
export default {
  ...
  onShow() {
   ...
   //    })
	...
	if (uni.getStorageSync("orderdetails")) {
		  var arr = uni.getStorageSync("orderdetails");
		  this.id = arr.id
		  this.registration.reportTime = arr.conductorConfirmTime;
		  this.registration.smsContent = arr.smsContent;
		  this.registration.sourceType = arr.sourceType;
		  this.registration.name = arr.name;
		  this.registration.insCorpCode = arr.insCorpCode;
		  this.insCorpName = arr.insCorpShortName;
		  this.registration.insCorpName = arr.insCorpShortName;
		  this.registration.model = arr.model;
		  this.registration.phone = arr.phone;
		  this.registration.plateNo = arr.plateNo;
		  this.registration.pushRepairType = arr.pushRepairType;
		  this.registration.pushRepairTypeName = arr.pushRepairTypeName;
		  this.registration.reportAddress = arr.reportAddress;
		  this.registration.reportNo = arr.reportNo;
		  this.current =  arr.canMoveCarStatus;
		  // this.registration.canMoveCarStatus = arr.canMoveCarStatus;
		  // this.canMoveCarList.map((item,index)=>{
		  // 	if(item.value == arr.canMoveCarStatus){
		  // 		this.current = item.value;
				
		  // 	}
		  // })
		  this.registration.carCanMoveStatus = arr.canMoveCarStatus;
		  // console.log(this.registration.CarCanMoveStatus)
		  this.registration.sourceTypeName = arr.sourceTypeText;
		}
  },
  ...
  methods: {
	radioGroupChange(e){
		...
		this.canMoveCarList.map(item=>{
			if(item.name == e){
				this.registration.carCanMoveStatus = item.value
			}
		})
    ...
    source_confirm: function(e) {
	  ...
	  this.registration.sourceTypeName = e[0].label;
    ...
    type_confirm: function(e) {
	  this.registration.pushRepairTypeName = e[0].label;
    ...
    btn_create() {
      ...
      // Indicator.open();
	    uni.showLoading();
      this.$http.put("/vi/repairOrder?id=" + this.id, registration).then(res => {
		uni.hideLoading();
